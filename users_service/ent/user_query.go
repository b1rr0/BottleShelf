...
func (uq *UserQuery) loadMemberships(ctx context.Context, query *MembershipQuery, nodes []*User, init func(*User), assign func(*User, *Membership)) error {
	...
	for _, n := range neighbors {
		fk := n.membership_member
		if fk == nil {
			return fmt.Errorf(`foreign-key "membership_member" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "membership_member" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
