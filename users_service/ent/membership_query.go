...
func (mq *MembershipQuery) QueryMember() *UserQuery {
...
func (mq *MembershipQuery) QueryOrg() *OrgQuery {
...
func (mq *MembershipQuery) WithMember(opts ...func(*UserQuery)) *MembershipQuery {
	query := (&UserClient{config: mq.config}).Query()
...
func (mq *MembershipQuery) WithOrg(opts ...func(*OrgQuery)) *MembershipQuery {
	query := (&OrgClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MembershipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Membership, error) {
	...
	if mq.withMember != nil || mq.withOrg != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withMember; query != nil {
		if err := mq.loadMember(ctx, query, nodes, nil,
			func(n *Membership, e *User) { n.Edges.Member = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withOrg; query != nil {
		if err := mq.loadOrg(ctx, query, nodes, nil,
			func(n *Membership, e *Org) { n.Edges.Org = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MembershipQuery) loadMember(ctx context.Context, query *UserQuery, nodes []*Membership, init func(*Membership), assign func(*Membership, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Membership)
	for i := range nodes {
		if nodes[i].membership_member == nil {
			continue
		}
		fk := *nodes[i].membership_member
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "membership_member" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MembershipQuery) loadOrg(ctx context.Context, query *OrgQuery, nodes []*Membership, init func(*Membership), assign func(*Membership, *Org)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Membership)
	for i := range nodes {
		if nodes[i].membership_org == nil {
			continue
		}
		fk := *nodes[i].membership_org
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "membership_org" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
